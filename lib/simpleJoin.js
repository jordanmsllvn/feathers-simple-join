                ...
                case 2:
                    ...
                    workingRecords = workingRecords.map(function (r) {
                        ...
                        if (options.through.attach && options.through.attach.length) {
                            r[options.with.as] = r[options.with.as].map(function (rm) {
                                var e_1, _a;
                                var throughRecord = joins_1.find(function (j) { return j[options.through.remote] === rm[options.with.remote] && j[options.through.local] === r[options.with.local]; });
                                try {
