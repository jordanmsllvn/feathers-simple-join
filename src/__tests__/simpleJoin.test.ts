...
app.service('a').create([
  {id:0, service: 'a', bId: 1},
  {id:1, service: 'a', bId: 1},
  {id:2, service: 'a', bId: 0}
]);

app.service('a-paginated').create([
  {id:0, service: 'a', bId: 1},
  {id:1, service: 'a', bId: 1},
  {id:2, service: 'a', bId: 0}
]);

app.service('b').create([
  {id:0, service: 'b'},
  {id:1, service: 'b'},
  {id:2, service: 'b'},
  {id:3, service: 'b'},
...
app.service('b-paginated').create([
  {id:0, service: 'b'},
  {id:1, service: 'b'},
  {id:2, service: 'b'},
  {id:3, service: 'b'},
...
test('Single result join works as expected on single record', async () => {
  ...
  const newResult = await simpleJoin(result, {with: {service: app.service('b'), as: 'joinedB', local: 'bId', remote: 'id'}});
...
test('Single result join works as expected on multiple (join table) records', async () => {
  ...
  const newResult = await simpleJoin(result, {
    with: {service: app.service('b'), as: 'joinedBs', local: 'id', remote: 'id'},
...
test('Multiple result join works as expected on single record', async () => {
  ...
  const newResults = await simpleJoin(results, {
    with: {service: app.service('b'), as: 'joinedB', local: 'bId', remote: 'id'},
  });
  expect(newResults.find((r: any) => r.id === 0).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.find((r: any) => r.id === 1).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.find((r: any) => r.id === 2).joinedB).toMatchObject({id: 0, service: 'b'});
})
...
test('Single result join works as expected on single record when records are paginated', async () => {
  const result = await app.service('a').get(0);
  const newResult = await simpleJoin(result, {with: {service: app.service('b-paginated'), as: 'joinedB', local: 'bId', remote: 'id'}});
  expect(newResult.joinedB).toMatchObject({
    id: 1,
    service: 'b'
  });
...
test('Multiple result join works as expected on single record when records are paginated', async () => {
  const results = await app.service('a').find();
  const newResults = await simpleJoin(results, {
    with: {service: app.service('b-paginated'), as: 'joinedB', local: 'bId', remote: 'id'},
  });
  expect(newResults.find((r: any) => r.id === 0).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.find((r: any) => r.id === 1).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.find((r: any) => r.id === 2).joinedB).toMatchObject({id: 0, service: 'b'});
...
test('Multiple result join works as expected on multiple (join table) records when records are paginated', async () => {
  const results = await app.service('a').find();
  const newResults = await simpleJoin(results, {
    with: {service: app.service('b-paginated'), as: 'joinedBs', local: 'id', remote: 'id'},
    through: {service: app.service('join'), local:'aId', remote: 'bId'}
  });
  expect(newResults[0].joinedBs.length).toBe(3);
  expect(newResults[1].joinedBs.length).toBe(3);
...
test('Multiple result join works as expected on single record when results are paginated ', async () => {
  ...
  expect(newResults.data.find((r: any) => r.id === 0).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.data.find((r: any) => r.id === 1).joinedB).toMatchObject({id: 1, service: 'b'});
  expect(newResults.data.find((r: any) => r.id === 2).joinedB).toMatchObject({id: 0, service: 'b'});
})
...
test('Include option works as expected on single record', async () => {
  const result = await app.service('a').get(0);
  const newResult = await simpleJoin(result, {with: {service: app.service('b'), as: 'joinedB', local: 'bId', remote: 'id'}, include: ['id']});
...
test('Exclude option works as expected on single record', async () => {
  const result = await app.service('a').get(0);
  const newResult = await simpleJoin(result, {with: {service: app.service('b'), as: 'joinedB', local: 'bId', remote: 'id'}, exclude: ['service']});
  expect(newResult.joinedB.id).toBe(1);
  expect(newResult.joinedB.service).toBeUndefined();
})
